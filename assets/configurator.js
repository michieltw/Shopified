...
const CONFIGURATOR_DATA = {
    options: [
        ...
        { id: 'hand', name: 'Stick hand', type: 'select', options: ['Left', 'Right'], default: 'Left' },
        { id: 'type', name: 'Stick type', type: 'select', options: ['Goalie', 'Player'], default: 'Player' },
        { id: 'category', name: 'Stick category', type: 'select', options: ['Senior', 'Intermediate', 'Junior', 'Youth'], default: 'Senior' },
        { id: 'shaft', name: 'Stick shaft', type: 'select', options: ['Goalie', 'Goalie Trigger', 'Junior', 'Youth', 'Oval', 'Square', 'Penta', 'Concave'], default: 'Square' },
        { id: 'flexProfile', name: 'Stick flex-profile', type: 'select', options: ['Ultra-low kick', 'Low kick', 'Mid-low kick', 'Hybrid kick', 'Tribrid kick', 'Mid kick', 'Elevated Mid kick', 'High kick'], default: 'Low kick' },
        { id: 'flex', name: 'Stick flex', type: 'select', options: ['20-50', '50-65', '65-125'], default: '65-125' },
        { id: 'bladecurve', name: 'Stick bladecurve', type: 'select', options: ['P02', 'P08', 'PM9', 'P14', 'P28', 'P28 Max', 'P31', 'P77', 'P86', 'P88', 'P90TM', 'P91A', 'P92', 'P92 Max', 'Full Custom (+€30)'], default: 'P92',
          surcharges: { 'Full Custom (+€30)': 30 }
        },
        { id: 'shaftTexture', name: 'Stick shaft texture', type: 'select', options: ['Matte', 'Matte + Tacky Grip', 'Glossy', 'Glossy + Tacky Grip', 'Super Glossy + Tacky Grip'], default: 'Matte + Tacky Grip' },
        { id: 'shaft3d', name: 'Stick shaft 3D texture', type: 'select', options: ['None', 'Fishbone', 'Straight', 'Diagonal', 'Fully Raised', 'Candy Cane'], default: 'None' },
        { id: 'bladeTexture', name: 'Stick blade texture', type: 'select', options: ['Matte', 'Glossy', 'Sanded', '3D texture'], default: 'Matte' },
        { id: 'colorstyle', name: 'Stick colorstyle', type: 'select', options: ['No color (Blackout)', 'Transparent', 'Painted'], default: 'No color (Blackout)' },
        { id: 'bladeColor', name: 'Stick blade color', type: 'select', options: ['Matches Shaft', 'Distinct'], default: 'Matches Shaft' },
        { id: 'weight', name: 'Stick weight class', type: 'select', options: ['Ultra lightweight (+€50)', 'Super lightweight (+€30)', 'Pro lightweight', 'lightweight', 'Welterweight'], default: 'Pro lightweight',
          surcharges: { 'Ultra lightweight (+€50)': 50, 'Super lightweight (+€30)': 30 }
        },
        { id: 'length', name: 'Stick length', type: 'select', options: ['Short', 'Standard', 'Extended (+€15)'], default: 'Standard',
          surcharges: { 'Extended (+€15)': 15 }
        },
        { id: 'namebar', name: 'Stick namebar + number', type: 'text', default: '' },
        { id: 'graphics', name: 'Stick graphics', type: 'select', options: ['No graphics (Blackout)', 'Pick a design', 'Custom design'], default: 'No graphics (Blackout)' },
        { id: 'mold', name: 'Stick mold', type: 'select', options: ['Premolded', 'Custom mold', 'Mystery mold', 'Pro range', 'Oilslick', 'Select', 'Cataphract', 'Houwitzer T7', 'Houwitzer T7K'], default: 'Premolded' },
    ]
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUI() {
        // Update Price
        if (priceDisplay) {
            // In a real Shopify setup, use Shopify.formatMoney
            priceDisplay.textContent = state.calculateTotal().toFixed(2);
        }
    ...
    if (btnCheckout) {
        btnCheckout.addEventListener('click', () => {
            // 1. Gather all line item properties from `state.state`
            // 2. Prepare payload for /cart/add.js
            // 3. fetch('/cart/add.js', { ... })
            // 4. Redirect to /checkout or open cart drawer
            alert("Shopify Checkout:\nWould POST to /cart/add.js with properties:\n" + JSON.stringify(state.state, null, 2));
        });
    }
